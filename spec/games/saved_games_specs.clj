...
(describe "Saves the Game:"
  ...
  (it "saves a file"
    (let [game {:status :spitting :database :file}]
      (should= (str game) (slurp (save/save-game game)))))

  (it "pulls recent or called for game"
    (let [game {:status :spitting :database :file}]
      (should= game (save/pull-game))))

  (it "ends a game in the middle"
    (let [game {:database :file :console :terminal :game-count 2 :users 1 :level :hard :current-player :player1 :box-played nil :player1 player1 :player2 player2 :board ["X" 1 "O" 3 4 5 6 7 8]}]
      (save/save-game game)))
  )