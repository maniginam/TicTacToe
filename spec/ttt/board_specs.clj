...
(describe "BOARD:"
  ...
  (it "checks diagonals for win"
    (should (is-diag-win? ["X" 1 2 3 "X" 5 6 7 "X"]))
    (should (is-diag-win? [0 1 "O" 3 "O" 5 "O" 7 "X"]))
    (should-not (is-diag-win? [0 1 "O" 3 "X" 5 "O" 7 "X"]))
    (should-not (is-diag-win? ["X" "O" "X" 3 4 5 6 7 8])))
  ...
  (for [[board results]
    ...
    (it (str "CHECKS " board " for " (if (true? results) " win" " no-win"))
...
(it "checks for existing & available box"
  ...
  (should= (str " 0 || 1\n===||=== \n 2 || 3\n") (draw-board {:console :terminal} [0 1 2 3 4 5 6 7 8])))
  )